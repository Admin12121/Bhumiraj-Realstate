...
function cleanNativeInputProps(
  props: Record<string, unknown>,
): React.InputHTMLAttributes<HTMLInputElement> {
  const entries = Object.entries(props).filter(
    ([key, value]) => key !== "tw" && value !== undefined,
  );
  return Object.fromEntries(entries) as React.InputHTMLAttributes<HTMLInputElement>;
}
...
export function Input({
  ...
  const nativeProps = cleanNativeInputProps(props as Record<string, unknown>);
    ...
    >
      {nativeInput ? (
        <input
          ...
          data-slot="input"
          {...(numericSize === undefined ? {} : { size: numericSize })}
          {...(nativeStyle === undefined ? {} : { style: nativeStyle })}
        />
      ) : (
        <InputPrimitive
          className={inputClassName}
