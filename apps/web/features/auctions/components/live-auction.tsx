...
function useCountdown(endsAt?: string, serverTime?: string) {
  ...
  const remaining = Math.max(
    0,
    (endsAt ? new Date(endsAt).getTime() : 0) - (now + offset),
  );
...
export function LiveAuction({ auctionId }: { auctionId: string }) {
  ...
  const countdown = useCountdown(snapshot?.endsAt, snapshot?.serverTime);
