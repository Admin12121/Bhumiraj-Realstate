...
export function PublicProfile({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    if (!node || !listings.hasNextPage) return;
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry?.isIntersecting && !listings.isFetchingNextPage) {
  ...
  }, [listings.fetchNextPage, listings.hasNextPage, listings.isFetchingNextPage]);
